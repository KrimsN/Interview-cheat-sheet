...
```mermaid
flowchart TB
    subgraph hierarchy["Иерархия (ромб)"]
        direction TB
        D1["D"] --> B1["B"]
        D1 --> C1["C"]
        B1 --> A1["A"]
...
```mermaid
flowchart TB
    subgraph mro["MRO — линейный порядок обхода super()"]
        direction LR
        m1["D"] --> m2["B"] --> m3["C"] --> m4["A"] --> m5["object"]
    end
```

Обратите внимание: из `B` вызов `super()` уходит в `C`, который **не является**
предком `B` — порядок задаёт MRO конкретного экземпляра, а не статическая
иерархия класса, где написан вызов.
...
(см. [Классы](classes-dataclasses-enum.md)). Ещё одна ловушка: если объявить
слот и дескриптор (например `@property`) с одним и тем же именем, они
конкурируют за один и тот же дескриптор на уровне класса — получится
`AttributeError` или property молча перекроет слот.
