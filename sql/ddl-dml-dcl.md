...
`TRUNCATE` быстрее, потому что не удаляет строки по одной. В PostgreSQL он всё
же журналируется через WAL — просто не построчно: в журнал попадает сама
операция усечения (фактически создание нового `relfilenode` для таблицы), а не
запись на каждую удаляемую строку, как у `DELETE`. Именно за счёт этого он
и работает так быстро.
