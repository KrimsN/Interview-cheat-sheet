...
- *ad hoc* — перегрузка функций по типам аргументов (Java, C++);
- *параметрический* — дженерики, один код для любого типа `T`;
- *подтипов (subtyping)* — вызов метода наследника через ссылку на базовый тип;
