...
- **`is` vs `==`** — `is` сравнивает идентичность объектов (адрес в памяти,
  фактически id()), `==` — вызывает `__eq__`. В python.md уже разобран
  частный случай с кешированием маленьких int'ов
  ([пример](../python.md#example-int-memory)) — стоит явно обобщить это в
