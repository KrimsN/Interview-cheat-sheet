...
В python.md уже разобраны продвинутые темы тайпинга — [PEP 695
generics/`Self`/`override`/отложенные
аннотации](../python.md#тайпинг-дженерики-self-override-отложенные-аннотации),
...
- **`Optional[X]` vs `X | None`** — `Optional[X]` — эквивалент `Union[X, None]`
  ...
  уже упомянуто в python.md в разделе про
  [Union-типы](../python.md#типы-данных)).
